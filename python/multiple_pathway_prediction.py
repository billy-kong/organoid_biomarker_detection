## Return predictive performance of multiple pathways
...
## Output file
fo_all = open('%s/multi_pathway_predictions.txt'%(tmp_dir), 'w')
print >> fo_all, '\t'.join(['cancer_type', 'drug', 'ML', 'num_pathways', 'pathways', '5yr_responder', '5yr_nonresponder', 'pvalue'])

fo_all_coef = open('%s/multi_pathway_coefficients.txt'%(tmp_dir), 'w')
...
for ML in ML_algorithm_list:
	...
	for cancer_type in test_types:
		...
		## Output files
		fo = open('%s/%s_%s.txt'%(fo_dir, cancer_type, drug), 'w')
		print >> fo, '\t'.join(['cancer_type', 'drug', 'num_pathways', 'pathways', '5yr_responder', '5yr_nonresponder', 'pvalue'])
		...
		for pathway_num in testing_pathways:
			...
			plt.tight_layout()
			plt.savefig('%s/%s_%s_%s_pathways.jpg'%(fo_dir, cancer_type, drug, pathway_num), format='jpg')
			plt.savefig('%s/%s_%s_%s_pathways.eps'%(fo_dir, cancer_type, drug, pathway_num), format='eps', dpi=300)
			plt.close()
			
			
			# output file
			print >> fo, '\t'.join(map(str, [cancer_type, drug, pathway_num,';'.join(map(str, features_used)), fiveYear_dic['Responder'], fiveYear_dic['Nonresponder'], pvalue]))
			print >> fo_all, '\t'.join(map(str, [cancer_type, drug, ML,  pathway_num,';'.join(map(str, features_used)), fiveYear_dic['Responder'], fiveYear_dic['Nonresponder'], pvalue]))
		fo.close()
fo_all.close()		
fo_all_coef.close()
